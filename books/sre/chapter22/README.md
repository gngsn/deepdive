...
1. 백엔드 한계가 **10,000 QPS**이며, 적절한 퇴보가 적용 후 모든 요청은 거부.
2. 프런트엔드가 `MakeRequest` 함수를 **10,100 QPS**의 비율로 일정하게 호출 시, 백엔드에 **100 QPS**의 과부하로 요청이 거부됨.
3. 실패한 **100 QPS**의 요청을 매 **1,000 밀리초** 마다 재시도하고 성공적으로 처리됨. 그러나 재시도로 인해 백엔드는 이제 **10,200 QPS**의 요청을 수신하게 되고 **200 QPS**의 추가 과부하가 생김.
4. 결과적으로 재시도 비율이 증가: 처음 **100 QPS**의 재시도가 **200 QPS**, 이어서 **300 QPS** 등으로 계속 증가하여 결국 백엔드가 처리할 수 있는 요청량이 줄어듦.
5. 백엔드 태스크가 더 이상 요청을 처리할 수 없게 되면 계속되는 요청과 재시도로 인해 충돌이 발생함. 
   - 처리되지 못한 요청들이 태스크에 부하를 가중시켜 이 태스크들 역시 과부하 상태에 놓이게 됨.

=> **재시도 때문에 시스템이 불안정해질 수 있음.**

- 백엔드가 요청을 처리하는 데 상당한 양의 자원을 소모한다면, 
  결국 과부하로 인해 요청 처리에 실패하게 되고 재시도 요청 때문에 백엔드는 지속적으로 과부하 상태에 놓이게 됨.
...
**캐시가 서비스에 중요한 영향을 미칠 때 고려해볼 것**:

- **서비스를 오버프로비전(overprovision) 함**
...
- 과부하 상태에서 서비스가 어떻게 동작하는지 이해하는 것이 중요
- 컴포넌트에 장애가 생길 때까지 부하 테스트 실행
  - 이상적인 컴포넌트는 성공적으로 처리하는 요청의 양이 크게 줄지 않으면서도 부하의 한계를 넘어선 요청에 대해서는 에러 혹은 경감된 결과를 리턴
- **부하의 갑작스러운 증가와 점진적 증가를 모두 테스트해야 함**
  - 캐시로 인해 다른 결과를 보일 수 있음
  - 각 컴포넌트마다 별도로 부하 테스트를 진행해야 함
  - 실제 트래픽을 테스트할 때는 충분한 여분의 수용량을 확보해야 하며, 수동으로 장애 조치를 수행해야 함
