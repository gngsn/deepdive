...
구글의 RPC 구현의 경우, 비활성화된 클라이언트(예: 활성화된 TCP 연결이 없는 클라이언트)는 주기적으로 UDP 기반의 상태 체크 요청을 전송함. 따라서 레임덕 정보가 모든 클라이언트에 신속하게(1~2 RTT 내) 전파됨
</pre>
...
대부분의 경우 30%의 서브셋 크기는 실전에서 사용하기에 이미 충분한 크기임.
...
- client[0], client[4], client[8]은 subset[0]을 사용
- client[1], client[5], client[9]은 subset[1]을 사용
- client[2], client[6], client[10]은 subset[2]를 사용
- client[3], client[7], client[11]은 subset[3]을 사용

클라이언트가 300개이고 각 클라이언트를 300개 중 10개의 백엔드에 할당하면, 앞서 예제의 실행 결과가 훨씬 더 좋아진다는 것을 알 수 있음.
